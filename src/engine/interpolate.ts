...
const DEFAULT: Required<NodeState> = { x: 0, y: 0, opacity: 1, scale: 1, rotate: 0 };

function resolve(s: NodeState | undefined): Required<NodeState> {
  return { ...DEFAULT, ...(s ?? {}) };
}
...
/** CSS transform string + opacity for a node state. */
export function toCss(state: NodeState): { transform: string; opacity: number } {
